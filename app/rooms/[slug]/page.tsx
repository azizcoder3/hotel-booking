...
export default async function RoomDetailPage({ 
  ...
  // 1. On attend les paramètres (Slug de la chambre)
  const resolvedParams = await params
  const { slug } = resolvedParams

  // 2. On attend les paramètres de recherche (Dates, Voyageurs)
  const resolvedSearchParams = await searchParams
  const { start, end, adults, children } = resolvedSearchParams

  const supabase = await createClient()
  
  // 3. Récupérer la chambre
  ...
  // 5. Récupérer les AVIS (avec le nom de l'utilisateur)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-6">
              {/* Le formulaire reçoit maintenant les dates de l'URL */}
              <BookingForm 
